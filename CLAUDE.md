...
**Calls are opened, edited, closed and deleted at `/admin/calls`** — the screen that stood between the lab and using any of this, since with no `cfp` row the homepage shows its empty state and `/proposals/new` has nothing to submit to. The list is the screen, in three sections (open, closed, deleted); `/admin/calls/<id>` edits one, through the same `CallForm.astro` the list's aside creates one with, and each card's actions sit in a `<details>` menu. It has no design of its own: artboard 1f draws the console's nav with a *Calls* link and never the screen behind it, so this is built in the same language rather than from a picture.
