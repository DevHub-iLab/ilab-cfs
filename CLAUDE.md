...
**Calls are opened, edited, closed and deleted at `/admin/calls`** — the screen that stood between the lab and using any of this, since with no `cfp` row the homepage shows its empty state and `/proposals/new` has nothing to submit to. The list is the screen, filtered to all, open, closed or deleted by a `.seg` of links — each view is a URL (`?show=open`), which is what lets `actionUrl()`/`backToCalls()` send every write back to the view it was posted from. `/admin/calls/<id>` edits one, through the same `CallForm.astro` the list's aside creates one with, and each card's actions sit in a `<details>` menu. It has no design of its own: artboard 1f draws the console's nav with a *Calls* link and never the screen behind it, so this is built in the same language rather than from a picture.
...
**Closing is `UPDATE cfp SET closes_at = now` and nothing else.** There is no status column and `src/lib/calls.ts` explains why one would only disagree with the deadline beside it. Three things in there are easy to get wrong from memory:

- The guard `closes_at is null or closes_at > now` on `close()` is what stops a double-submitted form rewriting when a call shut. Editing carries no such guard, for the reason `save()` in `proposals.ts` gives — an edit has no window to lose.
- A `datetime-local` posts a bare wall clock, so the `+08:00` in `parseSgt` is what keeps a deadline typed as 11:59pm from being stored as 7:59am SGT on a Worker whose local zone is UTC. It also refuses a day that does not exist, because V8 rolls `2026-02-30` over to 2 March rather than failing.
- **`readCallForm`'s `allowPast` is the whole difference between the two screens.** Creating a call already shut is a typo; on an existing one a past deadline is what every closed call has, so refusing it would mean a closed call could never be renamed. That is also why reopening needs no button: moving the deadline forward, or switching to *never closes*, is the only thing that decides it.

**Deleting a call is soft.** `remove()` stamps `cfp.deleted_at`, `restore()` clears it, and no `DELETE` is ever issued against `cfp` — the calls list grows a *Deleted* section with a Restore beside each one. It is still refused for a call anything has been pitched to: nothing would be destroyed, but those pitches would be left pointing at something the committee can no longer see while their authors go on reading the call's name.

**That is what `takingSubmissions(now)` in `src/lib/calls.ts` exists for.** A live call is now two conditions, not one — `deleted_at is null` *and* `closes_at is null or closes_at > now` — and the homepage, `/proposals/new`, the guarded insert in `proposals.ts` and `close()` all have to run both. The predicate is a Drizzle condition, so it drops into a builder's `where` and interpolates into a raw `sql` template alike. **A new page that filters calls uses it rather than writing the test out**; half the predicate fails silently, by advertising calls that were deleted.
