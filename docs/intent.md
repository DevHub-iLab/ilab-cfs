...
- **Render mode.** Pages render on demand unless opted out with `export const prerender = true`. The CFP landing page and code of conduct are prerendered. The schedule and archive are not — speakers may edit a scheduled talk and the page must reflect it. Confirm the adapter's `output` default when scaffolding rather than assuming it.
