...
- **Render mode is static by default** — verified at scaffold time, and the opposite of what Cloudflare's guide claims. `output` stays `"static"`; a route is server-rendered only when it declares `export const prerender = false`, and the build flips to server mode as soon as one route does. The code of conduct needs nothing. The homepage, the schedule, the archive, and everything behind auth must declare `prerender = false` — **a page that forgets it ships as a build-time snapshot**, which on an authenticated or live page is a correctness bug rather than a performance one.

  **The homepage sat on the static side of that line until 2026-09-13.** This paragraph read "the CFP landing page and code of conduct need nothing", and that was true of the page being imagined: prose about the lab and a link to submit. The page that was built is not that one. It lists the open calls straight out of `cfp`, with the days remaining beside each deadline, so prerendered it would go on advertising a closed call until somebody redeployed, and the countdown would be wrong from the first minute. The correction is worth keeping because the original was not a careless line — it was the right call for what it described. **What decides this is not how static a page feels but what it reads: a page may be prerendered only if nothing on it comes from the database or the clock.**
